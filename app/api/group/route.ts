...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, family, word, familyId, updates } = body;
...
export async function PATCHG(request: Request) {
  try {
    const body = await request.json();
...
export async function DELETE(request: Request) {
  try {
    ...
    data.families = data.families.filter((fam) => fam.id === familyId);
